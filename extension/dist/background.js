...
(async () => {
        ...
        try {
            if (method === 'Network.requestWillBeSent') {
                if (cdpNetworkRequests.size >= MAX_CDP_REQUESTS) {
                    ...
                    if (oldest)
                        cdpNetworkRequests.delete(oldest);
                }
    ...
    async function cdp(tabId, method, params = {}, timeout = 25000) {
        ...
        return await Promise.race([
            ...
            new Promise((_, reject) => setTimeout(() => reject(new Error(`CDP timeout: ${method} (${timeout}ms)`)), timeout)),
        ]);
    }
    ...
    wsConnection.registerCommandHandler('reloadExtension', async (params) => {
        const extensions = await chrome.management.getAll();
        const ext = extensions.find((e) => e.name.toLowerCase().includes((params.extensionName || '').toLowerCase()) &&
            e.installType === 'development');
        if (!ext)
            throw new Error('Extension not found or not unpacked');

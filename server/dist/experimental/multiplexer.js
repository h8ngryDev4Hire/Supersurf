...
const log = (0, logger_1.createLog)('[Mux]');
...
class Multiplexer {
    ...
    peers = new Map();
    leaderTabOwnership = new Map();
    leaderAttachedTabId = null; // leader session's attached tab ID
    leaderGroupId = null;
    // Scheduler state
    requestQueue = new Map();
    sessionOrder = []; // round-robin ordering
    currentSessionIdx = 0;
    processingQueue = false;
    currentExtensionTabId = null; // what tab the extension actually has attached
    ...
    constructor(port = 5555, host = '127.0.0.1', sessionId) {
        this.port = port;
        this.host = host;
    ...
    async startAsLeader() {
        this.extensionServer = new bridge_1.ExtensionServer(this.port, this.host);
        // Intercept raw connections — route peers by URL path, pass extension through
        this.extensionServer.onRawConnection = (ws, request) => {
    ...
    async handlePeerMessage(peer, data) {
        try {
            const msg = JSON.parse(data.toString());
            // JSON-RPC request from peer — enqueue it
            if (msg.jsonrpc === '2.0' && msg.method && msg.id !== undefined) {
                this.enqueueRequest(peer.sessionId, msg.method, msg.params || {}, msg.timeout || 30000, (result) => {
    ...
    enqueueRequest(sessionId, method, params, timeout, resolve, reject) {
    ...
    getSessionAttachedTabId(sessionId) {
    ...
    setSessionAttachedTabId(sessionId, tabId) {
        if (sessionId === this.sessionId) {
            ...
            const peer = this.peers.get(sessionId);
            if (peer)
                peer.attachedTabId = tabId;
        }
    }
    setSessionGroupId(sessionId, groupId) {
        if (sessionId === this.sessionId) {
     ...
     */
    filterTabsForSession(sessionId, tabs) {
        const allOwnedTabIds = new Set();
        for (const [sid, ownedTabs] of this.leaderTabOwnership) {
            if (sid === sessionId)
                continue;
        ...
        return tabs.filter((tab) => !allOwnedTabIds.has(tab.id));
    }
    // ─── Follower Mode ────────────────────────────────────────────
    ...
    async startAsFollower() {
        return new Promise((resolve, reject) => {
            ...
            this.leaderSocket = new ws_1.WebSocket(url);
            ...
            this.leaderSocket.on('message', (data) => {
                try {
                    ...
                    if (msg.type === 'tab_info_update') {
                        if (this.onTabInfoUpdate)
                            this.onTabInfoUpdate(msg.tabInfo);
                        return;
                    }
                }
                catch (error) {
            ...
            this.leaderSocket.on('error', (error) => {
    ...
    async attemptPromotion() {
        ...
        try {
            ...
            if (this.extensionServer) {
                ...
                this.extensionServer.onTabInfoUpdate = (tabInfo) => {
                    if (this.onTabInfoUpdate)
                        this.onTabInfoUpdate(tabInfo);
                    // Relay to peers
                    for (const peer of this.peers.values()) {
                        if (peer.ws.readyState === ws_1.WebSocket.OPEN) {
    ...
    sendCmdAsFollower(method, params, timeout) {
        if (!this.leaderSocket || this.leaderSocket.readyState !== ws_1.WebSocket.OPEN) {

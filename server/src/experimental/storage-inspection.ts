...
export const storageInspectionSchema: ToolSchema = {
  ...
  description:
    'Inspect and modify browser storage (localStorage/sessionStorage). Requires the `storage_inspection` experiment to be enabled.',
  inputSchema: {
    type: 'object',
    properties: {
      type: {
 ...
 */
export async function onBrowserStorage(
  ctx: ToolContext,
  ...
  if (!['localStorage', 'sessionStorage'].includes(storageType)) {
    return ctx.error(
      `Invalid storage type: "${storageType}". Must be "localStorage" or "sessionStorage".`,
      options
    );
  }
  ...
  if (!['get', 'set', 'delete', 'clear', 'list'].includes(action)) {
    return ctx.error(
      `Invalid action: "${action}". Must be one of: get, set, delete, clear, list.`,
      options
    );
  }
